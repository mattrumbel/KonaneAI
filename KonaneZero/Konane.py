...
class Game:
    ...
    def r_eval(self):
        ...
        m = self.getMoveablePieces(self.player_turn)
        n = self.getMoveablePieces(1 - self.player_turn)
        if n == 0:
            return float("inf")
        elif m == 0:
            return float("-inf")
    ...
    def movable_players_eval(self):
        global static_eval_count
        static_eval_count += 1

        my_moves = self.generate_legal_moves(self.player_turn)
        opp_moves = self.generate_legal_moves(1 - self.player_turn)
        ...
        if len(opp_moves) == 0:
            ...
            my_movable_pieces = list()
            opp_movable_pieces = list()

            for move in my_moves:
                if not(move[0] in my_movable_pieces):
                    my_movable_pieces.append(move[0])
            for move in opp_moves:
                if not(move[0] in opp_movable_pieces):
                    opp_movable_pieces.append(move[0])

            return len(my_movable_pieces) / len(opp_movable_pieces)
...
def minimax(alpha=float("-inf"), beta=float("inf"), depth_bound=0, game=Game()):
    ...
    eval_fs = {'naive': game.naive_evaluation,
               'q': game.q_eval,
               'r': game.r_eval,
               'moveable': game.movable_players_eval,
               'legal': game.legal_moves_eval}
    if depth_bound == 2:
        static_eval_count += 1
        # Second slot doesn't matter when at depth cap
        return eval_fs['legal'](), None
    else:
        opt_move = None
        minimax_calls += 1
        if game.player_turn == game.ai_player:
            ...
            # print("This is the found optimal move:", opt_move)
